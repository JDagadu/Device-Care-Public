...
export default function buildCautionDialog(cmessage){
    if(document.querySelector('#caution-modal')){
        document.body.replaceChild(buildCaution(cmessage),document.querySelector('#caution-modal')) ;
    }else{
        document.body.appendChild(buildCaution(cmessage));
    }
   
}

function buildCaution(cmessage){
